...
- Clarion's pre-SEI entity id is `{plugin_id}:{kind}:{canonical_qualified_name}`, **derived from the name + module path** and upserted on that key. A **rename or cross-module move changes the id** — and every binding keyed on the old id is **orphaned**. There was no rename detection, no lineage, no surrogate id (ADR-003 explicitly **deferred** an `EntityAlias` mechanism).
...
Every conforming subsystem is held to the same proven bar — including ones that feel done (Filigree), the authority itself (Clarion), and ones admitted later (Legis, Charter). Two rules follow:
...
| **Clarion + Filigree** (issues bound to live code) | association keyed to the entity | orphans without SEI |
...
| **Clarion + Charter** (requirements traced to live code) | trace link keyed to the entity | trace orphans on refactor |
...
> This is a historical intake snapshot kept for provenance; current per-subsystem status lives in each member's repo. As of 2026-06-02 all then-conforming subsystems had reported and Clarion's open items (REQ-C-01/02) were resolved in ADR-038. The standard is substantially **built**:
> - The shared **§8 conformance oracle EXISTS** (`~/clarion/docs/federation/fixtures/sei-conformance-oracle.json`).
> - **Clarion** (authority) shipped SEI end-to-end (migrations `0004_sei_prior_index` + `0005_sei`, `clarion-storage/src/sei.rs`, a passing `sei_conformance_oracle.rs`, the git-rename signal, the HTTP callers/callees linkages). **ADR-038 is Accepted.**
...
4. **Clarion is the authority.** Identity is minted, persisted, re-bound, and resolved in one place. Consumers never derive or parse it.
...
| **content_hash** | the per-entity content hash Clarion computes (`entities.content_hash`, the entity body) | changes on edit | the **freshness** signal |

SEI is **opaque**: consumers MUST NOT parse it. Its internal form is Clarion's business (REQ-C-02, ADR-038): collision-free under locator reuse, with no time/RNG component. The SEI *value* is reproduced from Clarion's persisted binding store, not re-derived; consumers never parse it regardless.
...
The matcher diffs against the previous index. So a load-bearing v1 obligation on Clarion is to **retain the prior SEI↔locator + body-hash + signature map** across re-index runs (a lightweight keyed side table, REQ-C-03). Its **durability is the real protection against cold-rebuild orphaning** — a `--force` that loses it re-mints every SEI regardless of token scheme.
...
- `_capabilities` advertises `sei: { supported: true, version: N }` so a consumer can detect a pre-SEI Clarion and **degrade** rather than guess.
...
- **No binding keyed on a locator, on any surface — [REQ-C-04].** No cross-tool binding may be keyed on a locator. Every Clarion surface returning an entity identity for use as a binding key — HTTP **and** MCP — MUST carry the **SEI**; a locator may also appear, but only explicitly labelled as the mutable address.
...
| **Filigree** (frozen) | **no code change, but not auto-conformant** (§0.1) — it already stores an opaque `clarion_entity_id`, so the standard only makes that stored value an SEI going forward, but conformance still requires the locator→SEI backfill (§7) and an oracle pass. Its `content_hash_at_attach` drift check now cleanly means the **content axis** (STALE); the identity axis (ORPHAN) lives in Clarion's `resolve_sei` |
...
| **Charter** | requirement trace links keyed on **SEI**; store SEI opaque, never mint/parse; mark links stale on lineage change; fall back to fragile file/symbol refs when Clarion absent. Declared in Charter ADR-005; adapter pending |
...
- **Legis** is both an SEI *consumer* (attestations key on SEI; lineage is its audit spine) and a *potential provider* of the git-rename signal the §3 matcher needs. v1 sources git-rename detection inside Clarion, but **behind a typed `git-rename signal` interface** (REQ-C-05) so it does not calcify as Clarion-internal. Clarion shipped the typed `GitRenameSource` trait + a `LegisGitRenameSource` consumer; Legis's provider half is contract-locked. Operative enablement is jointly gated on Clarion driving a committed rev-range.
- **The dossier** (Wardline) keys its sections on SEI and uses the §2.1 two-axis model.

## 7. Migration (one-time)

When Clarion first runs SEI-aware, it mints an SEI for every current entity. A backfill resolves every existing Filigree association and Wardline fact from its stored locator to the corresponding SEI and re-keys it. Locators that no longer resolve are **flagged ORPHAN for human review — never silently dropped**.
...
- **Hard cutover.** Clarion mints SEIs and the per-producer backfills run as one coordinated release step. Every federation feed emits **only locators before the cutover and only SEIs after it** — never a mix.
...
A shared, fixtures-based conformance suite every tool runs against a reference Clarion (`~/clarion/docs/federation/fixtures/sei-conformance-oracle.json`):
...
- **capability-absent fixture:** Clarion without the `sei` capability → consumers degrade honestly
...
| Lineage integrity | append-only store; consumer re-establishes integrity at its own boundary | Clarion-side **signed / hash-chained** lineage |
