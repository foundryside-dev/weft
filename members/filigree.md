...
**Surface facts (snapshot 2026-06-05 — NOT authoritative here; see the repo):** v2.3.0, shipped; ~115 MCP tools; HTTP `classic` + `loom` generations; web dashboard. Authoritative: `~/filigree/README.md`, `~/filigree/pyproject.toml`, `~/filigree/docs/mcp.md`.

## What it owns (authoritative in Filigree)

Issues & workflow state machines, dependencies/critical-path, observations (14-day TTL), files & scan-findings, the `entity_associations` table (opaque entity ids), and the **named HTTP generations** (`classic` `/api/*`, `loom` `/api/loom/*`). Route shapes, tool lists, and workflow packs are Filigree's authority — this hub does not restate them.
...
- Entity-association routes are **classic `/api/issue/…` + `/api/entity-associations`**, *not* `/api/loom/…` (old hub error — [conflict-register.md](../conflict-register.md) §A-8).
