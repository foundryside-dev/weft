...
**Repo:** `~/clarion` · **Language:** Rust
**Surface facts (snapshot 2026-06-05 — NOT authoritative here; see the repo):** v1.3.0, shipped (v1.0/1.0.1/1.1.0 tagged); ~39 consult-mode MCP tools; CLI + MCP (stdio) + loopback HTTP read API. First-customer target: elspeth (~425k LOC). Authoritative: `~/clarion/Cargo.toml`, `~/clarion/README.md`.

## What it owns (authoritative in Clarion)

Entity extraction (3-segment **locator** `{plugin_id}:{kind}:{qualname}`), edges, subsystem clustering, briefings, guidance, the `content_hash` (BLAKE3 entity-body), and — load-bearing for the suite — **SEI minting/persistence/resolution/lineage**. Tool names, ADRs, and route shapes are Clarion's authority.
...
- The asterisk for Clarion's plugin importing `wardline.core.registry.REGISTRY` is **RETIRED 2026-06-05** ([asterisk-register.md](../asterisk-register.md) A-2).
