...
- Planted structural findings for **Clarion** (dead entity, circular import in `specimen/dead_code.py`, `cycle_a.py`/`cycle_b.py`).
- Tracked work in **Filigree** (`.filigree/`), Clarion analysis (`.clarion/`), Wardline baseline (`.wardline/`). Legis/Charter wiring is design-only so far.

## What it demonstrates (the matrix, end to end)

Lacuna is where the [federation-map](../federation-map.md) becomes runnable: Wardline+Clarion dossier reads, Wardline→Filigree findings-as-work, Clarion+Filigree issues bound to SEI with drift detection, and (where live) the Legis/Charter cells. It is the thing a newcomer runs to *see the suite work*.

## Relationship to "elspeth"

Clarion names **elspeth** (~425k LOC real Python) as its first-customer scale target. Lacuna is the *small, deliberately-flawed* demo specimen — complementary, not the same role. Lacuna is the showcase; elspeth is the scale proof.
