...
**Current details:** package v1.0.0rc4 on the rc5 branch; semantic-tainting static analyzer; SARIF v2.1.0 plus native Filigree scan-results and Legis artifact outputs; an **8-state taint lattice** (not a "Tier 1–4" model), **3 canonical decorators** (`external_boundary`, `trust_boundary`, `trusted`), and the `PY-WL-*` rule set. For the latest details, use `~/wardline/src/wardline/{_version.py,core/taints.py,decorators/__init__.py,scanner/rules/,core/filigree_emit.py,core/legis.py}` and `~/wardline/README.md`.

## What it owns (authoritative in Wardline)

The trust lattice and its states, the decorator/annotation vocabulary, the rule set and rule IDs, the scanner engine, the `weft.toml [wardline]` + `.weft/wardline/` configuration/state surface, the runtime descriptor enforcement, and the reference corpus used to validate the analyzer. **Rule counts, decorator names, and lattice states are Wardline's responsibility — this hub points to the owning repo** (the old hub restated them and was wrong: [conflict-register.md](../conflict-register.md) §A-5).
...
- **Asterisk A-1 (LIVE):** Wardline's native Filigree emitter has shipped (`core/filigree_emit.py`, `POST /api/weft/scan-results`). The asterisk remains live only until the Loomweave-absent Wardline+Filigree composition is demonstrated end to end ([asterisk-register.md](../asterisk-register.md)).
