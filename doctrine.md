...
Per-member version and status are **snapshots, not authoritative here** — each member's authoritative status lives in its own repo. See [members/](./members/) for the current pointer-backed snapshot of each. In summary as of 2026-06-14: Loomweave, Filigree, Wardline, and Legis are built and in use and constitute the four-member launch cutover; Warpline is an admitted member (2026-06-14, PDR-0022 / §7 quality bar) with its implementation an admitted fast-follow outside that cutover; Charter is a designed-and-scaffolded member with federation adapters pending; Shuttle is a roadmap thought-bubble with no repo, lowest priority, displaceable by any better idea.
