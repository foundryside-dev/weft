...
> **Roster note (a snapshot, not the identity).** The 2026-06-05 ruling admitted Legis and Charter into the Weft family and excluded Shuttle as a member. Current taxonomy as of 2026-06-15: the suite *currently comprises* Loomweave, Filigree, Wardline, Legis, and Warpline (Warpline admitted 2026-06-14, PDR-0022 / §7 quality bar; pulled into the five-member launch cutover per PDR-0026, 2026-06-16), Plainweave (the reframed, renamed Charter) admitted 2026-06-24 as the code-grounded-intent authority (PDR-0030 — realized core + live-peer-validated seam, adapters maturing, not in the cutover), Tabard gap-named (§9), and Lacuna as the demo specimen. **This roster is open-ended** — it will grow (PDR-0024; daughter members) and the count is a current-state fact, never the suite's identity (the seam is). See [conflict-register.md](./conflict-register.md) §B-1 for the original ruling and its evidence.
...
- **Loomweave** — structural truth about the codebase, and the **identity authority for code and entities** of the suite (the *what* coordinate of the identity triad, below). Answers "what is this codebase, where should I touch, and what is the durable identity of this entity?" Owns the entity catalog, the code graph, guidance sheets, and the [Stable Entity Identity](./sei-standard.md) (SEI) every cross-tool binding keys on.
...
- **Warpline** — temporal and change-impact authority (the *when* coordinate of the identity triad, below — a temporal-*correlation* authority, not an identity authority). Answers "what changed, when, and what does this change touch?" Owns the change timeline, per-entity churn, impact radius, and the reverify worklist, keyed on Loomweave's SEI so its temporal facts survive rename and move.
- **Tabard** *(seventh member by gap-naming — see §7; the position and name are seated, the implementation is a Phase-0 spike, not yet built and not in the launch cutover)* — **actor-identity authority**: provable, unforgeable authorship (the *who* coordinate of the identity triad, below). Answers "who is this actor, and is this binding genuine?" Certifies an `actor_id ↔ public_key` binding that agents mint themselves and consumers verify **offline**, with no call to Tabard at verify time; it *attributes*, it never *gates*, and it is always enrich-only (absence degrades every consumer to free-text with `identity_stable:false`). Peer to Loomweave's identity authority — neither nested. See [members/tabard.md](./members/tabard.md).
- **Plainweave** *(admitted 2026-06-24, PDR-0030 — the reframed, renamed successor to Charter; realized core + live-peer-validated seam, federation adapters maturing, not in the launch cutover)* — **code-grounded intent**: the traceability graph that gives code its *reason to exist*. Answers "why does this entity exist, what requirement does it satisfy, and what goal does that requirement serve?" Every code SEI ladders up to a requirement and every requirement to a goal; an unlinked node at any level is a reviewable *"why does this exist?"*. Owns the requirement corpus and its bindings (requirements/traceability/baselines/verification evidence); advisory by default; a read-only consumer of its peers' surfaces. See [members/plainweave.md](./members/plainweave.md).
...
The sketched Shuttle scope is deliberately narrow: receive a scoped change intent, bind it to files or entities, order the edits, apply them incrementally with pre/post checks, roll back on failure, and lint / commit / emit telemetry on success — without planning, triaging, or reasoning about the code. It is named only so the change-execution gap is acknowledged. Plainweave (code-grounded-intent / requirements authority) is a *separate, realized* member and is not a substitute for it; do not conflate the two.

**The identity triad.** Identity in Weft is a coordinate space, not a single authority. Every federation event can carry three coordinates, each owned by exactly one authority and none nested: ***what*** changed (**Loomweave** / SEI — a *minted* identifier), ***who*** changed it (**Tabard** — a *certified* credential), and ***when*** it changed (**Warpline** — which *correlates* an existing `branch@sha` git value rather than minting an identity). So: two identity authorities (Loomweave, Tabard) plus one temporal-correlation authority (Warpline). The framing is deliberately coordinate-extensible, and the "one seam supported by a suite" positioning (§1) does not collapse the coordinates into one identifier. Do **not** flatten the triad to "three identifiers": *what* is minted, *who* is certified, *when* is an existing value correlated.
...
- **Plainweave** states and tracks code-grounded intent whether peers are installed or not — with manual trace links and file/symbol refs in solo mode, and SEI-keyed links, work binding, and finding references when peers are present.
...
### Naming a gap is not admitting an implementation (owner ruling, 2026-06-16)

Admission has **two stages**, and they must never be conflated:

1. **Gap recognition + naming.** The federation acknowledges a structurally
   *unowned coordinate*, reserves its bounded domain, and gives its authority a name
   and a roster seat. This says only: "the suite needs *something* to own this, and
   here is what it is called." It is owner-reserved, and it is **all that is required
   to be on the roster.** Functionally it is the honest act of *recognising we have a
   gap and giving it a name* — not a claim that anything is built.
2. **Implementation membership.** The *built thing* clears the quality bar above
   (standalone parity + federation enhancement + honesty conformance, **all in shipped
   behaviour**) and joins the working suite — e.g. a launch cutover.

The first stage neither implies nor shortcuts the second. A named-but-unbuilt seat
carries **no** claim to having met the quality bar; the bar gates the implementation in
full. This guards two failures at once: **name-sniping** a coordinate the suite clearly
needs (the seat can be reserved before the build exists), and a candidate invoking
"we're an admitted member" to **dodge the shipped-behaviour bar** (the seat grants
nothing the build has not earned). A reserved seat whose implementation never clears the
bar is a *documented gap*, not a member tool, and is named honestly as such.

**Tabard was admitted by stage 1 (2026-06-16):** the *who* coordinate of the identity
triad (§2) was structurally unowned — SEI keys *what*, nothing keyed *who*, and
Wardline's own `attest.py` states the gap verbatim — so the position and name are seated.
Its implementation is a Phase-0 go/no-go spike with no code yet, and it is **not** in the
five-member launch cutover.

> **PROVISIONAL — the HOME is open (2026-06-16, later same day).** What is named is the
> unowned *who* coordinate; **where it lives is not settled.** A subsequent design move —
> the hierarchical **session-prefix identifier** (`model+body+time` ± ticket) — reduces most
> of "Tabard" to a cross-member *stamping convention* + a small certification *Seal*, which
> reopens whether *who* needs a **separate seventh member** at all (vs folding the Seal into
> Loomweave or Legis). The **gap-naming holds regardless**; the home (separate member / fold /
> convention-plus-Seal) is decided by the Phase-0 spike + the key-custody review. Read the
> "seventh member" framing as the *maximal* option, not a settled one. See
> [pm/2026-06-16-background-ticket-change-debt-IDEA.md](./pm/2026-06-16-background-ticket-change-debt-IDEA.md).

## 8. Naming

Member products are named from weaving mechanics and adjacent registers — Loomweave, Filigree, Wardline, Legis, Warpline, Plainweave (reframed/renamed from Charter), Shuttle — as distinct proper names rather than subdivisions. There is no "Weft Guard," "Weft Workflow," or "Weft Execute"; each product earns its own identity. The family name sits above the products without dominating them, and — per §3 and §6 — it does not name any component that gets installed or runs.
...
Per-member version and status are **snapshots, not authoritative here** — each member's authoritative status lives in its own repo. See [members/](./members/) for the current pointer-backed snapshot of each. In summary as of 2026-06-16: Loomweave, Filigree, Wardline, Legis, and Warpline constitute the **five-member launch cutover** (Warpline pulled in per PDR-0026, 2026-06-16, reversing PDR-0022's fast-follow-outside posture; its build-out matures to launch-grade baseline in parallel). Loomweave, Filigree, Wardline, and Legis are built and in use; **Plainweave** (the reframed, renamed Charter) is admitted 2026-06-24 (PDR-0030) as the code-grounded-intent authority — realized core + a live-peer-validated seam, federation adapters maturing, not in the cutover; Shuttle is a roadmap thought-bubble with no repo, lowest priority, displaceable by any better idea.

**Tabard** is the **seventh member by gap-naming** (2026-06-16, §7): the *who* coordinate (actor-identity authority) is named and seated, but its implementation is a Phase-0 go/no-go spike — no code yet — and it is **not** in the five-member launch cutover; it matures separately and must clear the quality bar in shipped behaviour before it joins the working suite. The accompanying §2/§6 re-scope (Loomweave = identity authority **for code/entities**; Tabard = peer **actor**-identity authority; Warpline = temporal-*correlation*, not an identity authority) is owner-endorsed with Loomweave's pre-consent. Roster membership and the launch-cutover set (5) therefore deliberately diverge (the roster now: the cutover 5 + **Plainweave** admitted 2026-06-24 + **Tabard** gap-named). **(PROVISIONAL, §7: the *who* coordinate is named for good, but whether it is a separate member or folds into Loomweave/Legis — possibly as mostly a stamping convention + a small Seal — is open pending the spike; the §2/§6 re-scope below assumes the separate-member maximal case.)**
