...
- **Reference:** **all four writers now conform** — loomweave (`instructions.rs`,
  fixed by `4b4cdda`), filigree, legis, and wardline. The cross-tool investigation (filigree
  ...
  **swallow a foreign block** across an unclosed own-open — **all since FIXED and
  ...
  each member repo. A later 2026-06-10 pass found loomweave too was only **PARTIAL**
  (not the "exemplary" its cell claimed) — it violated (c) on the well-formed replace
  path and never implemented (e)/(h)/(g) — now fixed by `4b4cdda`
  (`clarion-35ae2fc514`); the "all four conform" claim holds only post-this-fix.*
...
| loomweave | **conforming** (rc3, commit `4b4cdda` + installed) — `crates/loomweave-cli/src/instructions.rs` | **Was PARTIAL, now FIXED.** The atomic temp-rename, the all-orphan malformed-recovery, and the namespaced `<!-- loomweave:instructions -->` block (`:v{version}:{hash}` provenance, both files, wired into `install --instructions` + `doctor`) were always foreign-safe — but the **well-formed replace path had zero foreign-fence awareness** (`locate_span` closed the span on the first own end-marker line), so (c) was **violated**: a filigree block sandwiched between a drifted own start and the own end was swallowed (reproduced live 1→0 against installed 1.1.0-rc3); and (e) own-duplicate canonicalisation, (h) the namespace-fence detector, and (g) an *explicit* refuse-to-empty guard were **not implemented**. Now satisfies (b)–(h): a case-insensitive `[A-Za-z0-9_-]+` fence parser (`parse_fence`), a real-foreign-block detector (`first_real_foreign_block_pos`, open+matching-close) + top-level own-open anchor (`first_own_open_fence_pos`) mirroring wardline; the replace path bounds at the first real foreign block and falls to bounded recovery when the own close lies beyond one (c); foreign-safe own-duplicate canonicalisation with a shielded duplicate surfaced via `InstructionsState::Duplicated` (e); explicit refuse-to-empty before `atomic_write` (g); foreign blocks never reordered (f). Fixed by `4b4cdda` (loomweave issue `clarion-35ae2fc514`, hub `weft-eb3dee402f`); full Rust floor green (1337 nextest incl. 4 new `instructions.rs` + 2 `doctor.rs` tests covering both `install --instructions` and `doctor --fix`; the two data-loss tests fail on pre-change code), live-verified via `doctor --fix`. **Caveat:** `rc3`-only — `main` lags pending the gated `rc3`→`main` merge (`weft-46f866cb85`); forward-port risk. *Corrects a prior cell that certified loomweave as pre-existing "exemplary / conforms" — it was partial on (c)/(e)/(h)/(g); marked FIXED, not pre-existing-conform. Source + test-verified 2026-06-10.* |
