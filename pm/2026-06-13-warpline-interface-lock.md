# Warpline Federation Seams — FROZEN Interface-Lock Specification
...
This document locks the **seam contracts** between warpline (the federation's 5th
member, the temporal / change-impact authority — "if I touch X, what breaks?")
and the four current members. warpline was admitted 2026-06-13 and pulled into the
...
inside loomweave/filigree/wardline/legis, warpline's internal storage schema
(`warpline_schema_version`, surfaced on every response — `commands.py:56`), and any
...
1. **Clean-break freeze.** Name + schema + error vocab pinned and versioned
   (`warpline.<contract>.v1`). No post-launch edits expected. A v2 is a NEW
   contract URI, never a mutation of v1 — siblings pin to a version.
2. **ENRICH-ONLY.** No seam makes a member's *core* flow depend on warpline.
   Every warpline-outbound consumption is an enhancement a member can omit.
   Every warpline-inbound read degrades to a coherent partial answer.
3. **NO-DEAD-BY-DESIGN.** We do not freeze an advertised-but-inert surface. The
   ...
   adapter ships.
4. **DECONFLICTION-FIRST.** warpline is deconfliction tooling, not security
...
5. **Expert-panel HARD RULE — loomweave retains no cross-run history.** The
   loomweave↔warpline seam is therefore strictly: **loomweave READS warpline's
   churn-count / timeline keyed by SEI and retains NOTHING.** loomweave never
   stores a warpline fact; it joins at read time and discards. This is the
...
6. **PROVEN-NEED GATE (owner 2026-06-13).** The hub supports what a member needs,
   ...
   loomweave inbound is **PROVEN** (warpline captures real edges through it) and
   frozen; the filigree / wardline / legis inbound reads are **RESERVED-SHAPE**
   (named, NON-binding on the sibling) until warpline demonstrates consumption in
   shipped behaviour. The overnight run is warpline's chance to prove them: a
   reserved-shape inbound seam freezes as a sibling obligation ONLY when a golden
   vector shows warpline actually reading it. We do not freeze a sibling obligation
   for a need warpline has merely claimed.

### The canonical success envelope (FROZEN)

Every warpline-outbound MCP tool returns `structuredContent` with this envelope.
...
`enrichment`, `meta.local_only`, `meta.peer_side_effects`) is warpline-side
...
- `data` is tool-specific and schema-bound (the `warpline.<contract>.v1` shape).
...
- `meta.local_only: true` and `meta.peer_side_effects: []` are an invariant for
  every read tool; only `warpline_edge_snapshot_capture` may carry a non-empty
  `peer_side_effects` and it is still `[]` because capture mutates only
  `.weft/warpline/` (`store.py:96-99`, `mcp.py:143-147`).

### The canonical error envelope (FROZEN)

Grounded in `federation-value-add-and-mcp-first-audit.md:107-145` and the live
`warpline.error.v1` emitter (`mcp.py:221-241`). JSON-RPC codes stay for protocol
...
> Note — live drift to reconcile pre-freeze (warpline-side, not a contract change):
> `mcp.py:221` currently emits `error_code: "invalid_params"`, which is NOT in
> the frozen vocabulary. The frozen set above is authoritative; warpline must map
...
loomweave. warpline stores it opaquely (`entity_keys.sei`, `store.py:21-31`) and
...
```json
{ "entity_ref":  { "kind": "auto|locator|sei|path|qualname|warpline_entity_key_id", "value": "..." } }
{ "changed_refs": [ { "kind": "locator", "value": "..." }, { "kind": "sei", "value": "loomweave:eid:..." } ] }
```

**Frozen keying rules:**

- Every warpline-outbound entity in `data` carries BOTH `locator` and `sei`
  (`sei: null` when warpline has not resolved one). This is already the store's
  read shape (`store.py:265-293` joins `ek.locator, ek.sei`).
- `warpline_entity_key_id` is accepted for compatibility but is NOT a federation
  key — it is a warpline-internal integer (`store.py:21`). Siblings MUST key on
  `sei` (preferred) or `locator` (fallback), never on `warpline_entity_key_id`.
  This closes the id-leak the morning audit flagged
  (`federation-value-add-and-mcp-first-audit.md:786-792`).
- **SEI-keyed facts depend on HX1.** The contract here SPECIFIES that warpline's
  facts are keyed by *real loomweave SEIs*. warpline resolving real SEIs (via
  `entity_resolve`, `loomweave.py:111-125`) is implementation ticket **HX1**;
  until HX1 lands, warpline emits `sei: null` + `enrichment.sei: "absent"` and
  remains locator-keyed. The contract does not change when HX1 lands — only the
  populated value does.

---

## SEAM 1 — warpline ↔ loomweave

**Authority split:** loomweave owns current structure, the entity catalogue, and
SEI minting/resolution. warpline owns temporal change facts and dated edge
...
### 1A. warpline OUTBOUND — tools loomweave (and other members) consume

#### `warpline_change_list`  (shim: `changed`)  — `warpline.change_list.v1`
...
```json
{
  "items": [
    {"change_id": "warpline:change:...",
     "entity": {"warpline_entity_key_id": 1, "locator": "python:function:src/pkg/mod.py::fn",
...
- `change_kind`, `actor`, `changed_at`, `commit`, `path` map 1:1 to
  ...
  falls back to `locator` (never `warpline_entity_key_id`).
- Required `next_actions`: `warpline_reverify_worklist_get`,
  `warpline_impact_radius_get`.

#### `warpline_entity_timeline_get`  (shim: `timeline`)  — `warpline.entity_timeline.v1`
...
```
- `sei_resolution` (resolved|unresolved|unknown) is a warpline-local honesty signal —
  does warpline hold a resolved SEI for this entity — NOT a loomweave lineage claim.
  loomweave owns lineage (`sei.rs:402-502`); warpline never restates it. (Renamed from
  `identity_status` per OD-4 to remove the lineage-authority ambiguity.)

#### `warpline_entity_churn_count_get`  (NEW — closes the critical gap)  — `warpline.entity_churn_count.v1`

**This read does not exist in warpline today and is the named gap the
...
cross-run history**. warpline is precisely the temporal authority that can supply
...
SEI warpline has never observed returns `churn_count: 0` (NOT an error — "no
...
### 1B. warpline INBOUND — the loomweave reads warpline captures from (MINIMAL)

warpline reads loomweave ONLY to capture dated edge snapshots and resolve SEIs.
These are the stable loomweave surfaces warpline needs and the frozen expectation
that loomweave keeps them. warpline calls them over loomweave's MCP
(`loomweave.py:73-108`, `snapshot.py:46-136`).

| loomweave MCP tool | warpline uses it for | Frozen shape warpline depends on |
| --- | --- | --- |
...
- warpline treats every loomweave fact as **dated enrichment**, stores only the
...
- A `truncated` neighborhood MUST NOT be snapshotted as complete
  (`snapshot.py:177-181` raises) — warpline records `DELTA`, never a false `FULL`.
- loomweave absence → warpline returns locator-keyed facts, `sei: unavailable`,
...
- loomweave absent on capture → `completeness: SKIPPED`, `enrichment.edges:
  skipped`, no edges stored (`snapshot.py:56-69`). `warpline_impact_radius_get`
...
- loomweave reading warpline (churn/timeline) absent → loomweave's `high_churn`
  ...
  loomweave→warpline direction.

### 1D. GOLDEN VECTORS — loomweave seam (freeze these)

| # | Producer | Consumer | Vector (frozen input → frozen output assertion) |
| --- | --- | --- | --- |
| GV-LW-1 | warpline `warpline_change_list` | loomweave temporal panel | rev_range `HEAD~1..HEAD` over a 2-entity change → `data.items` len 2, each item carries `locator`+`sei` keys, `changed_refs` SEI-keyed, `next_actions` names `warpline_reverify_worklist_get`. |
| GV-LW-2 | warpline `warpline_entity_churn_count_get` | loomweave `entity_high_churn_list` | input 3 SEIs, one never-observed → `data.items` len 3, observed two carry `churn_count>=1`, the unobserved carries `churn_count: 0` (not omitted, not error). |
| GV-LW-3 | warpline `warpline_edge_snapshot_capture` | warpline store | loomweave available, full neighborhood → `completeness: FULL`, `edges>0`; loomweave absent → `completeness: SKIPPED`, `edges: 0`, `enrichment.edges: skipped`, exit 0. |
| GV-LW-4 | loomweave `entity_neighborhood_get` | warpline capture | `truncated: {callers: true}` → warpline records `DELTA`, never `FULL` (`snapshot.py:177-181`). |
| GV-LW-5 | loomweave `entity_resolve` | warpline HX1 | locator `python:function:m.f` resolves → warpline stores `sei: loomweave:eid:...`, `enrichment.sei: present`; loomweave absent → `sei: null`, `enrichment.sei: unavailable`. |

---

## SEAM 2 — warpline ↔ filigree

**Authority split:** filigree owns work state, issues, observations, claims,
lifecycle. warpline reads or proposes links; never files, closes, claims, or
...
### 2A. warpline OUTBOUND — the reverify worklist filigree consumes

#### `warpline_reverify_worklist_get`  (shim: `reverify`)  — `warpline.reverify_worklist.v1`
...
"unavailable"`. warpline NEVER auto-files: any proposed work lands in
...
### 2B. warpline INBOUND — filigree work-state read (for reverify → worklist)

warpline reads filigree to answer "is this changed entity already tracked?" The
...
| filigree read | warpline uses it for | Frozen shape warpline depends on |
| --- | --- | --- |
...
**Frozen inbound rules:**
- warpline keys filigree reads on the **SEI** (the opaque `entity_id` ADR-029
  passes through; project isolation is by DB file). warpline supplies the SEI; it
  does not interpret filigree's `content_hash_at_attach` drift detection — that
  is filigree's read-path concern.
- filigree absent → warpline omits work enrichment, sets `enrichment.work:
...
| --- | --- | --- | --- |
| GV-FI-1 | warpline `warpline_reverify_worklist_get` | filigree closure-check evidence | rev_range over a changed entity with a linked P1 issue (filigree present) → item `priority: P1`, `enrichment.work[0].issue_status` populated, `next_actions.filigree` is a *candidate* (warpline filed nothing). |
| GV-FI-2 | warpline `warpline_reverify_worklist_get` | filigree | filigree absent → `enrichment.work: []`, envelope `enrichment.work: "unavailable"`, worklist still non-empty and useful (solo reverify). |
| GV-FI-3 | filigree `entity_association_list_by_entity` | warpline | `entity_id = loomweave:eid:X` with one bound issue → warpline work-enrichment shows that `issue_id`; SEI X with no binding → `enrichment.work: []` (not error). |

---

## SEAM 3 — warpline ↔ wardline

**Authority split:** wardline owns trust policy, findings, waivers, baselines,
the taint lattice. warpline consumes risk/finding facts as enrichment; **never
...
### 3A. warpline OUTBOUND — the affected-scope hint wardline consumes

#### `warpline.affected_scope.v1`  (payload, surfaced via `warpline_impact_radius_get`)

warpline exposes a changed+affected entity set with completeness/staleness for
wardline to scope a rescan (`federation-value-add-and-mcp-first-audit.md:507,515-517`).
This rides on `warpline_impact_radius_get` (shim `blast_radius`):

#### `warpline_impact_radius_get`  (shim: `blast_radius`)  — `warpline.impact_radius.v1`
...
- `completeness` + `staleness` are **MANDATORY in every answer** — a thin answer
  ...
  this scope MUST surface warpline's completeness/staleness in its scoped-scan
  metadata (`post-admission-consumer-tickets.md:27-30`).

### 3B. warpline INBOUND — wardline finding/risk read (advisory enrichment only)

| wardline read | warpline uses it for | Frozen shape warpline depends on |
| --- | --- | --- |
...
- warpline re-derives any "security" finding as a functional/availability signal
  for *ordering* reverify work; it is never an allow/deny gate.
- wardline absent → `enrichment.risk: unavailable`. warpline NEVER treats absence
...
| # | Producer | Consumer | Vector |
| --- | --- | --- | --- |
| GV-WL-1 | warpline `warpline_impact_radius_get` (`affected_scope`) | wardline scoped rescan | depth-2 over a changed entity with a FULL snapshot → `affected` non-empty, `completeness: FULL`, `staleness.commits_behind: 0`; wardline scan echoes that scope+staleness. |
| GV-WL-2 | warpline `warpline_impact_radius_get` | wardline | no snapshot → `completeness: NO_SNAPSHOT`, `changed` only, exit 0; wardline falls back to full scan (authoritative). |
| GV-WL-3 | wardline finding read | warpline reverify | wardline absent → `enrichment.risk: "unavailable"`, items still ordered by depth; NEVER an item marked clean/allowed. |

---

## SEAM 4 — warpline ↔ legis

**Authority split:** legis owns governance, signoff, CI/git attestations,
overrides, and the rename feed. warpline consumes provenance + rename facts and
...
### 4A. warpline OUTBOUND — advisory preflight impact legis consumes

#### `warpline.preflight_impact.v1`  (payload, surfaced via `warpline_impact_radius_get`)

warpline exposes an advisory impacted-entity set for governance/preflight context
(`federation-value-add-and-mcp-first-audit.md:508,518`). Same wire shape as
`warpline_impact_radius_get` (§3A), consumed as advisory: legis shows warpline
completeness/staleness next to identity gaps and lineage integrity
(`post-admission-consumer-tickets.md:21-24`) and **never lets warpline decide a
governance verdict**.

### 4B. warpline INBOUND — legis git-rename feed + provenance read

The load-bearing inbound read is the **git-rename feed**, so warpline's `timeline`
...
> **no** rename feed over MCP. So warpline's locator-rename source is an OPEN fast-follow
> choice — **C′** (loomweave grows a new MCP rename feed) or **A′** (warpline ports
> loomweave's derivation onto legis's path feed) — to settle when warpline proves the
> need. The shapes below are corrected to legis's real (path) output.

| legis read | warpline uses it for | Frozen shape warpline depends on |
| --- | --- | --- |
...
- legis supplies **path**-renames (`{old_path, new_path}`). The path→locator
  ...
  warpline consumers, with loomweave the single derivation authority. The proven consumer
  of legis's path feed today is loomweave (`LegisGitRenameSource`), at analyze time.
- legis absent → warpline uses raw git history, sets `enrichment.governance:
...
| # | Producer | Consumer | Vector |
| --- | --- | --- | --- |
| GV-LG-1 | warpline `warpline_impact_radius_get` (`preflight_impact`) | legis preflight context | depth-2 advisory set → legis surfaces it next to identity-gap/lineage with warpline staleness; legis policy decision UNCHANGED by it. |
| GV-LG-2 | legis `git_rename_list` (path) → loomweave-owned derivation | warpline timeline | a `{old_path, new_path}` rename, run through loomweave's path→locator derivation (mechanism C′/A′ TBD — see §4B amendment), yields the new locator's timeline incl. pre-rename events; legis absent → raw-git fallback, `enrichment.governance: unavailable`. |
| GV-LG-3 | warpline outbound | legis | every warpline response carries `meta.local_only: true`, `peer_side_effects: []` — legis verifies warpline never claims a write/govern side effect. |

---

## 5. Bonus seam — `warpline_edge_snapshot_capture` (the only mutating tool)

#### `warpline_edge_snapshot_capture`  (shim: `capture_snapshot`)  — `warpline.edge_snapshot.v1`
...
```
**Frozen rules:** mutates ONLY `.weft/warpline/` (`store.py:96-99`,
...
> Live drift to reconcile (warpline-side): `mcp.py:148-157` still accepts
...
| --- | --- | --- |
| `warpline_change_list` name + I/O + `warpline.change_list.v1` | ✅ | filters/sort/cursor population, real SEI keying (HX1) |
| `warpline_entity_timeline_get` name + I/O | ✅ | filters, `sei_resolution` population |
| `warpline_entity_churn_count_get` name + I/O (NEW) | ✅ | the GROUP-BY read in warpline; loomweave `high_churn` consuming it |
| `warpline_impact_radius_get` name + I/O (= `affected_scope` + `preflight_impact`) | ✅ | risk/depth sort, SEI-keyed inputs |
| `warpline_reverify_worklist_get` name + I/O | ✅ | risk/work/req/gov enrichment population |
| `warpline_edge_snapshot_capture` name + I/O + idempotency | ✅ | `idempotency_key`/`dry_run`/`mode` enforcement; drop `loomweave_command` |
| Success envelope (full shape) | ✅ | `query`/`next_actions`/`enrichment` population beyond thin shell |
| Error vocab (`warpline.error.v1`, retryability, error_code set) | ✅ | map live `invalid_params` → specific codes |
| SEI keying (`loomweave:eid:`, opaque, both `locator`+`sei` in every entity) | ✅ | HX1: warpline resolves real SEIs |
| loomweave inbound reads (`entity_resolve`, `entity_neighborhood_get`) | ✅ FROZEN — PROVEN (warpline captures real edges through it) | — already shipped in loomweave |
| filigree inbound (`entity_association_list_by_entity` SEI reverse-lookup) | ✅ FROZEN — EARNED (warpline reads it via `entity_association_list_by_entity` + `issue_get`; GV-FI-1/3 pass; byte-for-byte contract test pins the response) | filigree **consumer SHIPPED in 3.0.0**: `warpline_worklist_ingest` files/links worklist items, explicit-action only (weft-74f1e0c331) |
| wardline inbound (finding/risk by SEI) | ⏸ RESERVED-SHAPE — proven-need pending | freezes when warpline demonstrates risk-sort consumption; else non-binding |
| legis inbound (`git_rename_list` **path**-rename feed; locator derivation is loomweave's) | ⏸ RESERVED-SHAPE — proven-need pending (warpline uses raw git today); legis shape is `{old_path, new_path}`, not locator | freezes when warpline proves consumption; warpline-facing locator source is the OPEN **C′/A′** choice (loomweave MCP feed vs warpline ports derivation). See §4B amendment + PDR-0021 |
| Contract resource URIs (`warpline://contracts/warpline.*.v1`) | ✅ names | resource serving |
| Consumer implementations in all 4 members | — | filigree consumer (`warpline_worklist_ingest`) **PULLED INTO 3.0.0** (warpline ready; owner call 2026-06-13); loomweave/wardline/legis consumers remain fast-follow (per weft-54192f9400) |

---

## 7. Freeze preconditions (warpline-side reconciliation, not contract changes)
...
1. **Error-code specificity** — map the live generic `invalid_params`
   ...
   authoritative.
2. **Drop `loomweave_command`** from `warpline_edge_snapshot_capture` public input
   (`mcp.py:153`); move to server/project config.
3. **Schema names** — promote `warpline.draft.<tool>.v1` (`mcp.py:170-177`) to the
   frozen `warpline.<contract>.v1` URIs; the draft prefix must not survive the
   freeze.
4. **`warpline_entity_churn_count_get` must exist** — it is the NO-DEAD-BY-DESIGN
   ...
   contract slice.
5. **Endorsed names + shims both present** — `warpline_*` endorsed names live
...
>
> - **OD-1 → FREEZE ALL SIX.** `warpline_entity_churn_count_get` is in the frozen
>   launch slice (no-dead-by-design fill for loomweave `high_churn`; trivial
>   GROUP-BY; golden-vectored GV-LW-2).
> - **OD-2 → DROPPED.** `warpline_project_context_get` is NOT in the frozen surface —
>   member-internal ergonomics, not a cross-member seam, not live.
> - **OD-3 → NAME RESERVED ONLY.** No warpline↔charter schema freeze; charter binds to
>   the already-frozen `warpline_impact_radius_get` shape on arrival (avoids the
...
> - **OD-5 → FOLD INTO GS-7.** warpline's 14 golden vectors join the existing
...
**OD-1 — Does the churn-count read (`warpline_entity_churn_count_get`) freeze in
...
frozen warpline tool at the cutover expands the frozen surface. *Call needed:*
...
**OD-2 — Is `warpline_project_context_get` in the frozen launch surface?**
...
**OD-3 — Charter is a future member; do we freeze the warpline↔charter seam now?**
The package reserves `warpline.obligation_impact_context.v1`
...
only — warpline's outbound impact shape is already frozen via
`warpline_impact_radius_get`; charter binds to it post-arrival with no warpline
edit.**

**OD-4 — `identity_status` on the timeline output: warpline-local honesty signal or
...
(`sei.rs:402-502`). There is a doctrine risk that an agent reads warpline's
`identity_status` as a lineage authority claim. *Call needed:* keep it (documented
as warpline-local "do I hold a resolved SEI" only) or drop it and let agents call
...
contract gate (GS-7 oracle), or a separate warpline gate?**
`11-interface-contracts.md:48-53` says warpline "would enter the oracle corpus
before its first frozen release." *Call needed:* fold warpline's 14 golden vectors
(§1D, 2C, 3C, 4C) into the existing four-member conformance oracle as a fifth
producer, or stand up a warpline-specific gate. **Recommendation: fold into the
...
feed warpline can route through.

**Still OPEN (fast-follow, gated on warpline proving the need):** the warpline-facing
locator-rename source — **C′** (loomweave grows a new MCP rename-feed tool) vs
**A′** (warpline consumes legis's path feed and ports loomweave's derivation). Build
nothing until warpline demonstrates consumption (prove-the-need). The legis→loomweave
...
**Reversal trigger:** revisit the C′/A′ choice only if warpline's own derivation
measurably diverges from loomweave's for the same rename → then C′ (loomweave
becomes warpline's single rename source via a new MCP tool).
