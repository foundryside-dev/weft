...
| A-4 | clarion "v1.0 walking skeleton, v0.1 in flight / 7 MCP tools" → **v1.3.0 shipped / 39 tools** | `~/clarion/Cargo.toml`, README | loom stops restating; [members/loomweave.md](./members/loomweave.md) snapshots + points |
...
**Owner / follow-up.** loom (declared). Downstream: Loomweave's founding doctrine is reduced to a pointer to [doctrine.md](./doctrine.md) (Part B of the cutover), so the divergent three-member framing no longer sits in a repo as a competing authority.
